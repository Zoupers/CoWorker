...
def _parse_tool_arguments(raw: str, tool_name: str) -> dict:
    try:
        ...
        logger.warning(
            f"Failed to parse tool call arguments for '{tool_name}': {raw!r}"
        )
...
class MiniMaxProvider(BaseLLMProvider):
    ...
    async def complete(
        ...
        if self._current_model in _THINKING_MODELS and thinking:
            kwargs["extra_body"] = {"reasoning_split": True, "thinking": "adaptive"}
        elif not thinking:
            kwargs["extra_body"] = {"reasoning_split": True, "thinking": "disabled"}
