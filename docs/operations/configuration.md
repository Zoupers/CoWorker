...
Coworker 可以从源码 checkout 或容器运行。常用配置可以通过首次初始化向导、`.env`
或环境变量提供，变量名使用双下划线分组；需要无人值守配置时可从仓库根目录的
`.env.example` 开始。
...
| 变量 | 默认值 | 说明 |
|---|---|---|
| `COWORKER_REPOSITORY_URL` | 官方 CoWorker GitHub 仓库 | 新容器工作区要克隆的兼容仓库或 fork；支持 HTTPS、SSH、本地路径和 `file://` |
| `COWORKER_REPOSITORY_REF` | 镜像对应 revision，或仓库默认分支 | 新工作区要检出的分支、tag 或 commit |
| `COWORKER_WORKSPACE_PATH` | `/workspace/CoWorker` | 容器内持久化 Git checkout 路径 |
| `COWORKER_STATE_PATH` | `/var/lib/coworker` | 链接为 checkout 中 `data/` 的持久化状态目录 |

仓库 URL 和 ref 只初始化空工作区，不会在重启或镜像升级时覆盖已有 checkout。将 URL
显式设为空时，容器会从镜像内源码创建一个没有远端的 Git 基线。HTTP(S) URL 不允许内嵌
用户名、密码或 token；私有仓库应使用 SSH agent、deploy key 或 credential helper。
Compose 会把可选的宿主 `.env` 注入容器，但不会把整个宿主仓库挂载到 `/app`。
